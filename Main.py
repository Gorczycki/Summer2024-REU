...
def stockbrownianmotion(S_0, stock_mu, W, stock_sigma, dt):
    ...
    t = numpy.linspace(0, T, time_steps)
    S = S_0 * numpy.exp((stock_mu - 0.5 * stock_sigma**2) * t + stock_sigma*W)



    #Now make GBM with S(t) equation
...
W = W_t(num_brownian_motions)

stock_prices = numpy.array([stockbrownianmotion(S_0, stock_mu, stock_sigma, W, dt) for i in range(num_brownian_motions)])
...
# Plot Wiener process
plt.subplot(3, 1, 1)
plt.plot(numpy.linspace(0, T, time_steps), W)
...
# Plot stock price
plt.subplot(3, 1, 2)
plt.plot(numpy.linspace(0, T, time_steps), stock_prices)
...
plt.subplot(3, 1, 3)
plt.plot(numpy.linspace(0, T, len(call_prices)), call_prices)
